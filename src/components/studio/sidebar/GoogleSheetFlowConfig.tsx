...
interface GoogleSheetFlowConfigProps {
  node: Node;
  updateNodeData: (nodeId: string, data: any) => void;
}

export function GoogleSheetFlowConfig({ node, updateNodeData }: GoogleSheetFlowConfigProps) {
  ...
  const mapping = node.data?.mapping || { A: "", B: "", C: "" };
  ...
  const handleFileSelect = (fileId: string, fileFileName: string) => {
    ...
    <div className="h-full overflow-y-auto p-6 space-y-6">
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-900">Google Sheet Configuration</h3>
            <p className="text-xs text-slate-600">Configure which sheet to write to and what data to send</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
        <p className="text-xs text-slate-700">
          ...
          When the workflow reaches this step, it will append the mapped data to the selected Google Sheet.
        </p>
      </div>
    </div>
  );
